/* Richmond BC Chinese food meta — shared static renderer.
...
function fillHeader() {
  const m = DATA.meta || {};
  document.title = m.title || "Richmond BC food meta";
  const t = $("#title");
...
function initHome() {
  ...
  if (banner) {
    ...
    banner.textContent = `${DATA.hubs.length} hubs · ${stallCount} stalls →`;
  }
...
function hubBlock(h) {
  ...
  if (stalls.length) {
    tree.appendChild(
      el("p", "stall-tree-label", `Stalls worth knowing (${stalls.length})`)
    );
    stalls.forEach((s) => {
    ...
    tree.appendChild(
      el("p", "stall-tree-label muted", "Stall-level entries pending research.")
    );
  }
