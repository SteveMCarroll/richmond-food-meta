// Compile eastside/data/** (YAML frontmatter + markdown body) into the site dataset.
// Usage: node scripts/compile.mjs   (run from the eastside-site/ directory)
// This build reads ONLY the Eastside source root. Never glob it together with
// the Richmond BC data at repo-root data/.
...
const dataDir = path.join(repoRoot, 'eastside', 'data');
...
// --watch: recompile whenever anything under eastside/data/ changes.
if (process.argv.includes('--watch')) {
  console.log('\nWatching eastside/data/ for changes... (Ctrl+C to stop)');
  let timer = null;
  const onChange = (_e, f) => {
